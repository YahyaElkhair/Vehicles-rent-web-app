// src/pages/manager/VehicleCreate.jsx
import React, { useState, useContext, useCallback, useEffect, useRef } from 'react';
import { FiArrowLeft, FiPlus, FiX, FiCamera, FiInfo, FiStar } from 'react-icons/fi';
import { FaStar, FaGasPump, FaCar, FaDollarSign } from 'react-icons/fa';
...
const VehicleCreate = () => {
    ...
    // Handle drag events with counter for nested elements
    ...
    const processFiles = useCallback((files) => {
        ...
        Array.from(files).forEach(file => {
            ...
            } else if (formData.images.some(f =>
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.type) {
            setErrors({ type: ['Vehicle type is required'] });
            setIsSubmitting(false);
            return;
        }
        ...
        try {
            ...
            // Append all form data except images and features
            Object.entries(formData).forEach(([key, value]) => {
                if (key === 'images' || key === 'features' || value === null || value === '') return;
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                        {imagePreviews.map((preview, index) => (
                                            <div
                                                ...
                                                className={`relative group border rounded-lg overflow-hidden transition-shadow ${dragIndex === index ? 'opacity-50 cursor-grabbing' : 'cursor-grab'}`}
                                            >
                                                <img
                                                    src={preview.url}
                                                    alt={`Preview ${index}`}
                        ...
                        {errors.images && (
                            <div className="mt-3">
                                {errors.images.map((error, index) => (
                                    <p key={index} className="text-sm text-red-600 flex items-start mt-1">
